...
SELECT
  ...
  0,
  0,
  'Revision reason: Matching genome still in progress
prueba9',
  1,
  'AlgB',
...
SET
  ...
  reported_TF = CASE WHEN reported_TF IS NULL OR reported_TF='' THEN 'AlgB' ELSE reported_TF END,
  ...
  submission_notes = CASE
    WHEN submission_notes IS NULL OR submission_notes='' THEN 'Revision reason: Matching genome still in progress
prueba9'
...
SELECT 'AlgB', 27, 'A member of the NtrC family, AlgB is a two-component response regulator. AlgB belongs to a subfamily of NtrC proteins which can activate promoters that use sigma factors other than sigma 54. In Pseudomonas aeruginosa, AlgB has been shown to activate the production of alginate [PMID::17981963].'
WHERE NOT EXISTS (
  SELECT 1 FROM core_tf WHERE lower(name)=lower('AlgB')
);

UPDATE core_tf
SET
  family_id = COALESCE(family_id, 27),
  description = CASE WHEN description IS NULL THEN 'A member of the NtrC family, AlgB is a two-component response regulator. AlgB belongs to a subfamily of NtrC proteins which can activate promoters that use sigma factors other than sigma 54. In Pseudomonas aeruginosa, AlgB has been shown to activate the production of alginate [PMID::17981963].' ELSE description END
WHERE lower(name)=lower('AlgB');
...
SELECT
  ...
  (SELECT TF_id FROM core_tf WHERE lower(name)=lower('AlgB') LIMIT 1),
  ''
WHERE NOT EXISTS (
...
SET
  TF_id = COALESCE(TF_id, (SELECT TF_id FROM core_tf WHERE lower(name)=lower('AlgB') LIMIT 1)),
...
VALUES
  ('Helicobacter pylori 26695', 'Helicobacter pylori 26695', NULL,
   0, NULL, 'Revision reason: Matching genome still in progress
prueba9',
...
SELECT 'CAT reporter gene assays', 'CAT reporter gene assays', NULL, 'ECO:0005617'
WHERE NOT EXISTS (
  SELECT 1 FROM core_experimentaltechnique WHERE EO_term='ECO:0005617'
);
