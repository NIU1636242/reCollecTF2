...
export default function Step4ReportedSites() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      for (const g of genomeList) {
        ...
        const seq = fastaText.replace(/>.*/g, "").replace(/[^ATCG]/gi, "").toUpperCase();
  ...
  function findGenesForHit(acc, hitStart, hitEnd) {
    ...
    const dist = g =>
      hitEnd < g.start ? g.start - hitEnd :
      hitStart > g.end ? hitStart - g.end : 0;
    ...
    const res = [];
    const push = g => !res.some(r => r.locus === g.locus) &&
  ...
  function isCompleted(site) {
    const c = choice?.[site];
    return c?.startsWith("ex-") || c?.startsWith("fz-");
  }

  function findExact() {
    const arr = rawSites.split(/\r?\n/).map(s => s.trim().toUpperCase()).filter(Boolean);
    setSites(arr);
    setActiveSite(null);
    ...
    arr.forEach(site => {
      ...
      genomes.forEach(g => {
        ...
        while (i !== -1) {
          all[site].push({ site, start: i, end: i + L - 1, acc: g.acc, strand: "+" });
          i = g.sequence.indexOf(site, i + 1);
        }

        let j = g.sequence.indexOf(rc);
        while (j !== -1) {
          all[site].push({ site, start: j, end: j + L - 1, acc: g.acc, strand: "-" });
    ...
    genomes.forEach(g => {
      for (let i = 0; i <= g.sequence.length - L; i++) {
        ...
        if (mismatches(sub, site) > 0 && mismatches(sub, site) <= 2)
          found.push({ site, match: sub, bars: buildBars(site, sub), start: i, end: i + L - 1, acc: g.acc, strand: "+" });
        if (mismatches(sub, rc) > 0 && mismatches(sub, rc) <= 2)
          found.push({ site, match: sub, bars: buildBars(rc, sub), start: i, end: i + L - 1, acc: g.acc, strand: "-" });
      }
    });
  ...
  function handleConfirm() {
    setStep4Data({ siteType, rawSites, sites, exactHits, fuzzyHits, choice, showFuzzy, activeSite });
    goToNextStep();
  }
    ...
    <div className="space-y-8">
      ...
      <div className="bg-surface border border-border rounded p-4">
        <button className="flex justify-between w-full font-semibold mb-3" onClick={() => toggleAcc("a1")}>
          <span>Reported sites</span><span>{accordion.a1 ? "▲" : "▼"}</span>
        </button>

        {accordion.a1 && (
          <div className="space-y-3 text-sm">
            <div className="space-y-1">
              <label className="flex items-center gap-2"><input type="radio" checked={siteType === "motif"} onChange={() => setSiteType("motif")} /> motif-associated (new motif)</label>
              <label className="flex items-center gap-2"><input type="radio" checked={siteType === "variable"} onChange={() => setSiteType("variable")} /> variable motif associated</label>
              <label className="flex items-center gap-2"><input type="radio" checked={siteType === "nonmotif"} onChange={() => setSiteType("nonmotif")} /> non-motif associated</label>
            </div>

            <textarea className="form-control w-full h-40" value={rawSites} onChange={e => setRawSites(e.target.value)} />
          ...
          {sites.map(s => (
            <div
              ...
              className={`cursor-pointer p-2 border-b last:border-0
                ${activeSite === s ? "bg-muted" : ""}
                ${isCompleted(s) ? "text-green-400 font-semibold" : ""}`}
            >
              {s}
            </div>
          ))}
      ...
      <div className="bg-surface border border-border rounded p-4">
        <button className="flex justify-between w-full font-semibold mb-3" onClick={() => toggleAcc("a2")}>
          <span>Exact site matches</span><span>{accordion.a2 ? "▲" : "▼"}</span>
        </button>
              ...
              return (
                <div key={site} className="border border-border rounded p-3 space-y-2">
                  ...
                  {arr.map((hit, i) => {
                    ...
                    return (
                      <label key={i} className="flex items-start gap-2 text-xs cursor-pointer">
                        <input type="radio" name={`ex-${site}`} checked={choice[site] === `ex-${i}`}
                          onChange={() => setChoice(p => ({ ...p, [site]: `ex-${i}` }))} />
                        <div>
                          ...
                          {nearby.length > 0 && (
                            <table className="mt-1 text-[11px]">
                              <thead>
                                <tr><th>locus tag</th><th>gene name</th><th>function</th></tr>
                              </thead>
                              <tbody>
                                {nearby.map((g, idx) => (
                                  <tr key={idx}><td>{g.locus}</td><td>{g.gene}</td><td>{g.function}</td></tr>
                                ))}
                              </tbody>
                            </table>
                          )}
                  ...
                  <label className="flex items-center gap-2 text-xs cursor-pointer mt-2">
                    <input type="radio" name={`ex-${site}`} checked={choice[site] === "none"}
                      onChange={() => { setChoice(p => ({ ...p, [site]: "none" })); findFuzzy(site); }} />
      ...
      {showFuzzy && (
        <div className="bg-surface border border-border rounded p-4">
          <button className="flex justify-between w-full font-semibold mb-3" onClick={() => toggleAcc("a3")}>
            <span>Inexact matches (mismatches)</span><span>{accordion.a3 ? "▲" : "▼"}</span>
          </button>
                ...
                return (
                  <div key={site} className="border border-border rounded p-3 space-y-2">
                    ...
                    {arr.map((hit, i) => {
                      ...
                      return (
                        <label key={i} className="flex items-start gap-2 text-xs cursor-pointer">
                          <input type="radio" name={`fz-${site}`} checked={choice[site] === `fz-${i}`}
                            onChange={() => setChoice(p => ({ ...p, [site]: `fz-${i}` }))} />
      ...
      <button className="btn mt-6" onClick={handleConfirm}>Confirm and continue →</button>
    </div>
  );
}
